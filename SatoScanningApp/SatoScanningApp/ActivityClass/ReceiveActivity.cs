...
namespace SatoScanningApp.ActivityClass
{
    ...
    public class ReceiveActivity : AppCompatActivity
    {
        ...
        TextView txtTotalQty, txtMsg, txtScanQty, txtFromPlant;
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            try
            {
                ...
                SetContentView(Resource.Layout.activity_directReceive);
                if (uhfAPI == null)
                {
                    try
                    {
                ...
                txtFromPlant = FindViewById<TextView>(Resource.Id.txtPlant);
        ...
        public void GetDocNumber()
        {
            try
            {
                string _MESSAGE = "IN_AND_OUT_RCV~BIND_DOC~" + clsGlobal.mPlant + "~" + "~" + "~" + "~" + "~" + "~" + "}";
        ...
        public void GetToPlant()
        {
            try
            {
                string _MESSAGE = "IN_AND_OUT_RCV~BIND_FROM_PLANT~" + clsGlobal.mPlant + "~" + "~" + cmbDocNo.SelectedItem.ToString() + "~" + "~" + "~" + "~" + "}";
                string[] _RESPONSE = oNetwork.fnSendReceiveData(_MESSAGE).Split('~');
                ...
                switch (_RESPONSE[0])
                {
                    case "VALID":
                        ...
                        for (int i = 0; i < sArr.Length; i++)
                        {
                            txtFromPlant.Text = sArr[i].ToString();
                        }
        ...
        private void GetDocDetails(string sRequestNo)
        {
            try
            {
                ...
                sRequest = oNetwork.fnSendReceiveData("IN_AND_OUT_RCV~BIND_VIEW~" + clsGlobal.mPlant + "~" + "~" + sRequestNo + "~" + "~" + "~" + "~" + "~" + "}");
        ...
        private string[] CompleteDispatchDataToServer()
        {
            try
            {
                string _MESSAGE = "IN_AND_OUT_RCV~COMPLETE~" + clsGlobal.mPlant + "~" + txtFromPlant.Text.Trim() + "~" + cmbDocNo.SelectedItem.ToString().Trim() + "~" + editAssetBarcode.Text.Trim() + "~" + clsGlobal.UserId + "}";
                string[] _RESPONSE = oNetwork.fnSendReceiveData(_MESSAGE).Split('~');
                return _RESPONSE;
            }
            catch (Exception ex)
        ...
        private string[] ScanDispatchDataToServer()
        {
            try
            {
                string _MESSAGE = "IN_AND_OUT_RCV~SAVE~" + clsGlobal.mPlant + "~" + txtFromPlant.Text + "~" + cmbDocNo.SelectedItem.ToString().Trim() + "~" + editAssetBarcode.Text.Trim() + "~" + clsGlobal.UserId + "}";
                string[] _RESPONSE = oNetwork.fnSendReceiveData(_MESSAGE).Split('~');
                return _RESPONSE;
            }
            catch (Exception ex)
